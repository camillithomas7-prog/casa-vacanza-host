...
$title = 'Noleggi · Auto, Golf Cart, Scooter, Monopattino';
$metaDesc = 'Noleggio auto, golf cart, scooter e monopattini elettrici a Sharm El Sheikh. Tariffe giornaliere, settimanali, mensili.';
...
<section class="relative isolate -mt-[68px] pt-[68px] overflow-hidden">
  ...
  <div class="container-wide pt-12 md:pt-16 pb-8">
    <div class="badge-brand mb-3"><i data-lucide="key-round" class="size-[12px]"></i> Noleggi</div>
    <h1 class="font-serif text-4xl sm:text-5xl md:text-6xl font-semibold tracking-tight text-balance">Muoviti come vuoi.</h1>
    <p class="text-ink-600 dark:text-ink-300 mt-3 text-lg max-w-xl text-pretty">Auto, golf cart per il resort, scooter e monopattini elettrici. Tariffe giornaliere, settimanali e mensili.</p>

    <div class="mt-8 flex flex-wrap gap-2">
      <a href="/noleggi.php" class="px-4 py-2 rounded-xl text-sm font-medium <?= !$type ? 'bg-ink-900 text-white dark:bg-white dark:text-ink-900' : 'bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-700 hover:bg-ink-50' ?>">Tutti</a>
      <?php foreach ($rentalTypes as $t):
        $count = (int)val('SELECT COUNT(*) FROM services WHERE type = ? AND active = 1', [$t]);
      ?>
        <a href="/noleggi.php?type=<?= e($t) ?>" class="px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-2 <?= $type === $t ? 'bg-ink-900 text-white dark:bg-white dark:text-ink-900' : 'bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-700 hover:bg-ink-50' ?>">
...
<section class="container-wide py-10">
      ...
      <a href="/noleggio.php?slug=<?= e($s['slug']) ?>" class="group block animate-slide-up" style="animation-delay:<?= $i * 50 ?>ms">
        <div class="aspect-[4/3] rounded-2xl overflow-hidden bg-ink-100 dark:bg-ink-900 relative shadow-card">
          ...
          <?php if ($s['resort_name']): ?>
            <div class="absolute top-3 right-3 badge bg-brand-500 text-white backdrop-blur">presso resort</div>
          <?php endif; ?>
        </div>
        <div class="mt-4">
          ...
          <div class="flex items-baseline justify-between mt-3">
            ...
            <?php if ($s['weekly_price']): ?><span class="text-xs text-ink-500">da <?= fmtMoney((float)$s['weekly_price']) ?>/settimana</span><?php endif; ?>
          </div>
        </div>
      </a>
    <?php endforeach; ?>
  ...
  <?php if (!$items): ?>
    <div class="card p-14 text-center">
      <div class="font-display font-bold text-xl">Nessun veicolo disponibile</div>
    </div>
  <?php endif; ?>
</section>
