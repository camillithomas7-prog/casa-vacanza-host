...
$title = 'Escursioni a Sharm El Sheikh';
$metaDesc = 'Escursioni guidate a Sharm El Sheikh: Ras Mohammed, Tiran, deserto in quad, beduini, Cairo, Monte Sinai. Prenota online.';
...
<section class="relative isolate -mt-[68px] pt-[68px] overflow-hidden">
  ...
  <div class="container-wide pt-12 md:pt-16 pb-8">
    ...
    <div class="mt-8 flex flex-wrap gap-2">
      <a href="/escursioni.php" class="px-4 py-2 rounded-xl text-sm font-medium <?= !$cat ? 'bg-ink-900 text-white dark:bg-white dark:text-ink-900' : 'bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-700 hover:bg-ink-50' ?>">Tutte</a>
      <?php foreach ($expTypes as $t):
        $count = (int)val('SELECT COUNT(*) FROM services WHERE type = ? AND active = 1', [$t]);
        if ($count === 0) continue;
      ?>
        <a href="/escursioni.php?cat=<?= e($t) ?>" class="px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-2 <?= $cat === $t ? 'bg-ink-900 text-white dark:bg-white dark:text-ink-900' : 'bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-700 hover:bg-ink-50' ?>">
...
<section class="container-wide py-10">
  ...
  <?php if (!$items): ?><div class="card p-14 text-center"><div class="font-display font-bold text-xl">Nessuna escursione disponibile</div></div><?php endif; ?>
</section>
